...
class Dataset_PartialSpoof_train(Dataset):
    ...
    """
    def __init__(self, list_IDs, seglab, base_dir, args, algo):
    ...
    def __getitem__(self, index):
        ...
        # Use empty bonafide fallback if utt_id not in seglab
        raw_lab = self.seglab.get(utt_id, np.ones(NUM_FRAMES, dtype=str))
        # Align: seglab is based on original audio length;
        # since we truncate audio to 4.175s, keep only first NUM_FRAMES labels
        frame_labels, boundary_labels = seglab_to_frame_labels(
            raw_lab, num_frames=NUM_FRAMES
        )

        return waveform, frame_labels, boundary_labels
        # shapes: [66800]  [208] long  [208] float32
